...
- **후속 수정 (codex TUI '움직임' 3차 — 전체 화면 고정)**: 헤드리스 미니 VT 파서로 codex가 뿌리는 **실제 화면**(120x30)을 재구성 — codex는 로딩 중 박스가 37→50칼럼으로 커지고 입력 라인(10→16행)·상태줄(13→20행)로 재배치하며, 하단 터미널 기본 높이가 codex가 필요한 **20행**에 못 미치면 박스/입력/상태가 잘려 '오가듯' 보임. 수정: ①`TerminalPanel`이 **최소 20행(`MIN_ROWS`)을 보장**하도록 패널 높이에 따라 폰트(9~14px) 자동 조절(히스테리시스: rows<20 줄임, rows>26 키움), ②하단 터미널 기본 높이 28%→36%(최소 200px). 160x35·220x45에서도 codex가 전폭이 아닌 고정폭(50칼럼)을 그리는 것을 확인(코드 크기와 무관) — 따라서 행 수 확보가 핵심
- **후속 수정 (codex 커서 '점프')**: 사용자 보고 — 화면 이동은 해결됐으나 codex 채팅창 안에서 커서가 계속 순간이동(3·4행 위아래로 튐). 헤드리스로 codex 커서 이벤트를 타임라인 캡처 → codex가 프레임을 그릴 때마다 `?25h` 직전 커서 위치가 **10행·11행·13행 등으로 매번 다름**(8초간 29회) — codex가 커서를 입력줄/상태줄 등 다른 위치에 올려놓는 codex 쪽 동작임을 확인 → 터미널 측에서 할 수 있는 것은 커서를 덜 띄우기뿐이라 `TerminalPanel`에 `cursorStyle: 'bar'`, `cursorBlink: false` 적용
- **후속 수정 (codex 커서 '점프' 2차 — CSS 숨김)**: 바/블링크 해제로는 여전히 순간이동(사용자 "그래도 똑같네"). 심층 캡처(커서 이벤트+sync 상태+직전 컨텍스트)로 확정 — codex는 프레임을 그린 뒤 **커서를 입력줄이 아니라 '마지막으로 칠한 위치'(11;3H·13;3H·3;1H 등)에 놓고 `?25h`로 보여주므로**, 렌더링 크기와 무관하게 xterm 블록이 프레임마다 그 위치를 쫓아 순간이동함(codex 쪽 버그, xterm 옵션으로는 불가). 해결: ①`TerminalPanel`이 사용자가 입력한 명령줄을 얕게 추적(ESC 시퀀스 스킵·백스페이스 처리)해 **`codex`(exec 제외) 실행 직후 `cs-hide-cursor` 클래스 부여** → `index.css`의 `.cs-hide-cursor .xterm .xterm-cursor { opacity:0; visibility:hidden }`로 xterm v6 DOM 커서(블록/바 모두) 숨김 — 입력 피드백은 codex가 입력줄을 직접 다시 그려 유지 ②복원: 출력 청크에서 셸 프롬프트(`[A-Za-z]:\...>`) 복귀 감지 또는 셸 종료(`onDone`) 시 클래스 해제
- **파일**: `src/components/TerminalPanel.tsx`, `src/index.css`, `README.md`
