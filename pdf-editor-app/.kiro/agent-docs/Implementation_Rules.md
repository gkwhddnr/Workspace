...
- **목적**: 두 개의 화살표가 끝점에서 연결되어 있을 때, Ctrl+클릭 또는 드래그-앤-드롭으로 연결 지점의 화살표 머리만 제거하고 하나의 연속된 선으로 만들기
- **동작**:
  1. 화살표 끝점을 Ctrl+드래그하여 다른 화살표 끝점에 가져다 대면 스냅(Snap) 발생
  2. 마우스 업 시 두 화살표의 포인트 배열을 하나로 병합 (전체 마디 유지)
  3. Chained Merge 지원: 1-2-3-4번 등 무제한 연쇄 병합 가능해야 함
- **Hit Test**: 마디가 여러 개인 화살표의 경우, 모든 마디(선분)에서 클릭 및 선택이 작동하도록 루틴을 돌아야 함
- **구현 주의**: 병합 작업 시 반드시 클래스 생성자(`new ShapeElement`)를 사용하여 인스턴스화할 것 (JSON 직렬화 시 메서드 유실 주의)
