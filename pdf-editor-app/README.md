...
- **codex 커서 '점프' 완화**: 헤드리스로 codex의 커서 이벤트를 타임라인으로 캡처한 결과, codex는 프레임을 그릴 때마다 **커서를 입력줄/상태줄 등 각기 다른 위치(8초간 29회, 10·11·13행)에 올려놓아** 커서가 튀며 순간이동해 보임(codex 쪽 동작) → xterm 커서를 `cursorStyle: 'bar'`, `cursorBlink: false`로 바꿔 눈에 덜 띄게 완화
