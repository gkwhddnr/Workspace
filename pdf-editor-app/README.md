...
- **겹침 병합**: 새로 그린 형광펜이 기존 형광펜과 겹치면(걸친 영역 포함) 합집합 윤곽 하나로 병합 (아래 사각형 병합과 동일한 `rectParts`/`outlineSegments` 구조)
...
- **대화형 특수키 전달 (화살표 선택 가능)**: 실행 중(busy) 화살표·Esc·Tab·Backspace를 PTY로 그대로 전달(`terminal:input`)하도록 추가 — `codex` 업데이트 안내처럼 **1/2/3 메뉴를 화살표로 이동**하고 Enter로 확정할 수 있습니다(입력이 있으면 Enter 시 한 줄 전송, 비어 있으면 `\r`만 전달). 로컬 입력창 히스토리(↑/↓)는 대기 상태에서만 동작. `node` raw 모드·Windows `choice`로 실제 키 전달(`[27,91,65]`/`[13]`) e2e 검증
- **터미널 렌더러를 xterm.js로 교체 (codex 등 전체 화면 TUI 정상 표시)**: 라인 기반 텍스트 뷰어는 codex의 브라유 스피너(`⠋⠙⠹`)·커서 이동/리드로우 이스케이프를 해석하지 못해 "모스부호"처럼 깨져 보였습니다 → `@xterm/xterm`(+`@xterm/addon-fit`) 도입으로 **PTY raw 바이트를 그대로 렌더링**(색상·커서·전체 화면 UI 정상). 키 입력(화살표·`Ctrl+C`·붙여넣기)은 xterm이 PTY로 직접 전달하고, 패널 크기 변화 시 `cols/rows`를 `terminal:resize`로 동기화. 기존의 별도 입력창·히스토리·busy/passthrough·`sanitize()` 로직을 제거해 구조가 단순해졌습니다
- **터미널 기본 실행 제거 (기본 닫힘 + 셸 지연 생성)**: 터미널을 플러그인으로 분리했는데도 앱을 켜면 PDF 하단 터미널이 열려 있고 셸 프로세스가 이미 떠 있던 문제 — `pdfTerminalOpen` 기본값을 `false`로 바꾸고, PTY 세션도 앱 시작 시가 아니라 **`terminal:start` 시점(터미널을 열거나 입력할 때)에만 스폰**하도록 지연 생성으로 변경했습니다. 플러그인 실행 뷰에서 터미널을 실행할 때만 셸이 시작됩니다
- **터미널 하단 노출을 플러그인 상태에 연동**: "터미널 플러그인을 비활성화했는데 아래 화면엔 터미널이 그대로 뜬다"는 불일치 해소 — **하단 임베드 터미널(접힘 상태의 '터미널 열기' 바 포함)은 터미널 플러그인이 활성화된 경우에만 렌더링**합니다. 비활성화하면 하단에서 완전히 사라집니다
- **codex 로딩 스피너(브라유) 폰트 폴백**: `codex`를 실행하면 채팅 커서가 계속 움직이는데 로딩 스피너(`⠋⠙⠹...`, Braille U+2800–U+28FF)가 **안 보이던 문제** — Consolas/Cascadia Mono에 브라유 글리프가 없어 빈칸으로 렌더링됐기 때문(헤드리스 캡처에서 6초간 49프레임 확인). xterm 폰트 체인에 **`"Segoe UI Symbol"`**을 추가해 스피너가 보이도록 수정
- **codex TUI '움직임' 개선 ① (크기 안정화 + 이중 렌더링 방지)**: ①터미널 셸을 **레이아웃 확정 후의 실제 크기(cols/rows)로 시작**하도록 지연 시작 — 크기가 불안정하게 전달되면 codex 같은 TUI가 리사이즈(윈도우 크기 변경)를 감지해 **전체 화면을 반복 재배치**하며 '채팅창과 터미널창을 오가는 듯' 보이는 문제 예방 ②하단 임베드 터미널과 플러그인 실행 뷰 터미널은 **PTY 세션이 하나**인데 둘 다 마운트되면 같은 출력이 두 패널에 이중 렌더링됨 → 플러그인 뷰에서 터미널이 열려 있으면(`terminalViewOpen`) 하단 임베드를 숨겨 중복 방지
- **codex TUI '움직임' 개선 ② (codex 전체 화면 고정)**: 헤드리스로 codex의 실제 화면을 재구성해 보니 codex는 로딩 중 **박스가 37→50칼럼으로 커지고** 입력(16행)·상태줄(20행)로 재배치합니다. 하단 터미널 기본 높이가 codex가 필요한 **20행에 못 미치면** 박스/입력/상태가 잘려 "채팅창↔입력창으로 오가는" 듯 보였음 → 터미널 패널이 항상 **최소 20행을 표시**하도록 폰트 크기를 패널 높이에 따라 자동 조절(9~14px, 히스테리시스로 리사이즈 대응, `MIN_ROWS=20`)하고, 하단 터미널 기본 높이를 28%→**36%**(최소 200px)로 상향
- **codex 커서 '점프' 해결(숨김)**: 이벤트 캡처 결과 codex는 프레임을 그린 뒤 **커서를 입력줄이 아니라 마지막으로 칠한 위치(10·11·13행)에 놓고 `?25h`로 보여줘** 블록 커서가 프레임마다 순간이동해 보임(codex 쪽 동작, 스타일 변경만으론 안 됨) → 사용자가 입력한 명령줄을 얕게 추적해 **`codex`(exec 제외) 실행 중에는 xterm 커서를 CSS로 숨기고**(`.cs-hide-cursor .xterm .xterm-cursor`), 셸 프롬프트(`D:\Workspace>`) 복귀 또는 셸 종료 시 복원. 입력 피드백은 codex가 입력줄을 직접 다시 그려 유지됨
- **터미널 분할·스레드 (멀티 세션)**: 터미널 안에 여러 셸을 "스레드"로 두고 한 화면에 분할 표시합니다. PTY 세션을 `sessionId` 단위로 관리하도록 main/preload 타입을 개편(단일 전역 세션 → `Map<sessionId, term>`, IPC·이벤트에 sessionId 포함, 신규 `terminal:destroy`)하고, 터미널을 **파네**(세션 1:1, 기존 xterm·codex 커서 숨김·MIN_ROWS=20 폰트 조절 유지) + **워크스페이스**(상단 스레드 탭 + 분할 버튼) 구조로 재구성 — **단일/좌우/상하/4분할**을 전환하면 부족한 칸에 새 셸이 자동 스폰되고, 탭 `X`로 스레드 종료, 포커스된 파네만 커서 수신. 터미널을 접거나 닫으면 소유 스레드가 일괄 정리됩니다
- **도구·터미널 도킹 위치 (왼쪽/오른쪽/아래)**: PDF 패널 안에서 **도구(Tools) 패널**과 **터미널**을 각각 왼쪽·아래·오른쪽 어디로든 도킹할 수 있습니다. Tools 헤더(하단·좌우 `PanelBottom/Left/Right` 아이콘 스위치)와 터미널 워크스페이스 바에서 위치를 바꾸면 **PDF 뷰어를 기준으로 즉시 재배치**(좌/우는 세로 분할, 아래는 가로 분할, 서로 다른 쪽이면 중첩 배치). 도킹 패널은 전용 드래그 핸들로 폭/높이를 조절할 수 있고, 터미널 접힘 상태는 도킹 방향에 맞춰 세로 스트립/가로 '터미널 열기' 바로 표시됩니다
...
- **터미널 플러그인 추가**: 시스템 셸을 다루는 **터미널**을 별도 빌트인 플러그인으로 분리 추가 — 설치된 플러그인 목록에서 활성화·실행하면 플러그인 실행 뷰에 터미널 패널이 열립니다(Electron IPC: `terminal:exec`/`terminal:kill`). `cmd /d /s /c`(Windows) 또는 `sh -c`(그 외)로 명령어를 한 줄씩 실행하며 실시간 출력, 명령 히스토리(↑/↓), `Ctrl+C` 명령 중단(taskkill `/t`), `Ctrl+L` 출력 지우기, `cd` 디렉터리 이동(세션 cwd 유지), 시스템 코드페이지(CP949 등) 자동 감지 디코딩으로 한글 출력을 지원
- **탭 화면 기본 분할 비율 재설정**: 크게 볼 탭(웹 서퍼·코드 에디터)과 작게 볼 탭(단축키·플러그인·AI 코파일럿 채팅)을 구분해 초기 폭을 웨이트 기반으로 분배 — 기타 그룹 내부는 웹:코드:단축키:플러그인 = **3:3:1:1**(예: 웹·코드 2개 열면 1:1), AI 코파일럿(플러그인 실행 뷰) 실행 시 최상위에서 기타:AI = 웨이트합:2로 배치해 AI 채팅 폭을 작게 유지, PDF 패널은 기존처럼 우선 확보(PDF+기타+AI = 44%:나머지 분배, PDF+AI만이면 80%:20%)
...
- **FactChat(금오공대 AI) 제공자 추가**: AI 코파일럿에 넷째 제공자로 **FactChat(금오공대 학교 AI 게이트웨이, OpenAI 호환)** 추가 — `callFactChat`이 `https://factchat.mindlogic-kr-api.com/v1/gateway/chat/completions/`로 호출(Bearer 인증, 키는 조직 범위), 설정 패널에서 API 키 입력 가능(`apiKey_factchat` localStorage 저장), 모델은 메타데이터(`/models/`) 실측 기준 Claude Sonnet 5(기본)·Opus 5·Fable 5, GPT-6 Astra·GPT-5.6 Sol, Gemini 3.8 Flash·3.1 Pro, Grok 4.6 중 선택, 플러그인 버전 2.2.0·단축키 안내 문구 갱신
...
### 2026-09-10
- **AI 코파일럿 모델 라인업 최신화**: Claude·ChatGPT(GPT)·Gemini 공식 문서 기준으로 최신 모델 반영
  - **Gemini**: 3.8 Flash(기본), 3.5 Flash, 3.1 Pro(Preview), 2.5 Flash
  - **ChatGPT**: GPT-5.6 Sol(기본)·Terra·Luna, GPT-5.5
  - **Claude**: Opus 5(기본), Sonnet 5, Haiku 4.5, Opus 4.8(Legacy)
  - 단축키 안내 및 플러그인 설명을 'Gemini · ChatGPT · Claude' 3중 엔진 지원으로 갱신, AI 코파일럿 플러그인 버전 2.1.0
- **AI 모델 선택 동작 수정**: 설정에서 고른 모델이 실제 요청에 반영되지 않던 문제 수정 — `callAi`에 선택한 모델 전달
- **Gemini 503 자동 재시도**: 신규 GA 모델 출시 초기 서버 과부하(503/502) 시 최대 3회 백오프 재시도, 503·404·429 오류를 한글 안내 메시지로 표시
- **웹뷰 `dom-ready` 예외 수정**: `loadURL()`이 `dom-ready` 이벤트 발생 전 호출되어 나던 예외 수정 — 준비 전 요청은 대기열에 보관 후 자동 로드
- **AI 코파일럿 플러그인 활성/비활성 토글 정상화**: 활성화 시 패널 즉시 표시·비활성화 시 패널 닫힘, 비활성 상태에서는 실행 차단(안내 알림), 헤더 'AI Pilot Live/Off' 배지를 플러그인 상태와 연동, 활성 상태를 재시작 후에도 유지(영속화)하도록 수정
- **AI 코파일럿 화면·파일 컨텍스트 공유**: 모든 AI(Gemini·ChatGPT·Claude)가 편집 중인 화면과 열린 파일의 **실제 내용**을 읽고 요약·분석할 수 있도록 컨텍스트 첨부 — 코드 에디터 전체 소스, 웹 서퍼 현재 페이지 본문(`<webview>` 텍스트 추출), 열린 PDF의 전체 추출 텍스트(신규 `PdfTextService`, 문서 캐시·크기 제한), 설정에서 공유 켜기/끄기 토글
- **AI 코파일럿 대화 스레드(저장 공간)**: 대화 내용이 localStorage(`aiThreads`)에 자동 저장되는 스레드 시스템 추가 — 헤더 스레드 메뉴에서 새 스레드 생성·전환·삭제, '새 대화' 제목은 첫 사용자 메시지로 자동 생성, 재시작 후에도 대화 유지, 헤더에 현재 스레드명 표시
- **AI 코파일럿 대화 스레드 백엔드 저장**: 대화 스레드를 백엔드(Spring, H2 DB `ai_thread` 테이블)에 저장하여 앱을 껐다 켜도 이어짐 — 신규 `AiThreadController`(`GET/POST/DELETE /api/pdf/ai-threads`, Upsert·삭제)와 `AiThreadService`(프론트 Axios) 추가, 메시지 입력 시 0.5초 디바운스로 자동 저장, 앱/패널 시작 시 백엔드 스레드를 로드해 로컬 상태와 동기화, 백엔드가 비어 있는 최초 실행은 기존 localStorage 스레드를 백엔드로 마이그레이션, 오프라인 시 localStorage 캐시 폴백
...
- **지우개 도구 드래그 삭제 기능 개선**: '클릭 삭제' 모드(OFF)에서도 마우스를 꾹 누른 채 드래그하면 연속적으로 삭제되도록 개선
- **PDF 저장 최적화**: 텍스트 원본 보존 및 투명 필기 이미지 임베딩(PNG) 방식으로 저장 로직을 전면 수정하여, 파일 용량 폭증을 방지하고 원본 텍스트의 인식·복사 기능을 완벽히 유지
